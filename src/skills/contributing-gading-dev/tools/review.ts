...
export const metadata = {
  name: "gading_dev_review",
  description: "Membuat branch baru, men-commit perubahan, push, dan membuat Pull Request (PR) ke main untuk direview",
  inputSchema: {
    type: "object",
    properties: {
      commitMessage: {
...
export async function handler(args: any) {
  ...
  // Run review sequence: branch, commit, push, create PR
  const steps = [
    `git checkout main`,
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Review/PR creation failed at step:\n\n${results.join("\n\n")}`,
          },
        ],
        isError: true,
      };
