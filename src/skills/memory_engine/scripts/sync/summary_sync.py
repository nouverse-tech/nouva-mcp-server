...
def generate_daily_summary(
    ...
    line listing Obsidian wikilinks (e.g. [[Gading]], [[Nouverse]]) built from the
    people and projects fields above.
    """
    payload = {
        "model": model,
...
def inject_related_dates(
    ...
    pattern_links = r"(🔗\s*Links:\s*|(?:\*\*|__)🔗\s*Links:(?:\*\*|__)\s*)(.*)"
    if re.search(pattern_links, body_text, re.IGNORECASE):
        body_text = re.sub(pattern_links, new_links_line, body_text, flags=re.IGNORECASE)
    else:
